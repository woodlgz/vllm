...
class BlockTable:
    ...
    def compute_slot_mapping_device(self, req_indices: np.ndarray,
        ...
        if self.dcp_world_size > 1:
            ...
            block_table_indices.mul_(self.self.max_num_blocks_per_req)
            block_table_indices.add_(positions.div(virtual_block_size, 
                                                   rounding_mode = "floor"))
            #block_numbers = self.block_table_np.ravel()[block_table_indices]
            block_numbers = torch.index_select(self.block_table, 
                                               dim = 0,
            ...
            slot_mapping = block_numbers * self.block_size + block_offsets
            # Write final slots, use -1 for not-local
            self.slot_mapping[:req_indices.shape[0]] = torch.where(
                mask, slot_mapping, -1)
        else:
            #block_table_indices = (req_indices * self.max_num_blocks_per_req +
            #                       positions // self.block_size)
            ...
            torch.add(block_numbers, block_offsets, 
                      out = self.slot_mapping[:req_indices.shape[0]])
